...
public class ViewDisciplanesList extends JPanel {
	 ...
	 * Create the panel.
	 */
	public ViewDisciplanesList(JPanel jPanel) {
		setSize(725, 484);
		...
		JLabel deleteLabel = new JLabel("Excluir Disciplina:");
				...
				try {
					...
					if(index < 1 || index > Database.disciplanes.size()) {
						...
						for(int i = 0; i < Database.professors.size(); i ++) {
							Database.professors.get(i).removeDisciplane(Database.disciplanes.get(index));
						}
						
						for(int i = 0; i <Database.turmas.size(); i++) {
							Database.turmas.get(i).removeDisciplane(Database.disciplanes.get(index));
						}
						
						Database.disciplanes.remove(index);
						
						jPanel.removeAll();
						jPanel.setVisible(false);
						jPanel.setVisible(true);
						jPanel.add(new ViewDisciplanesList(jPanel));
					}
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if(index < 1 || index > Database.disciplanes.size()) {
						...
						jPanel.setVisible(false);
						jPanel.setVisible(true);
						jPanel.add(new ViewDisciplane(Database.disciplanes.get(index - 1)));
					}
				}catch(NumberFormatException i) {
		...
		DefaultTableModel tableModel = new DefaultTableModel(
				...
				new String[] {
					"", "Disciplina", "Professores", "Max. Professores", "Turmas"
				}
			);
		...
		for(int i = 0; i < Database.disciplanes.size(); i++) {
			tableModel.addRow(new Object[] {i+1, Database.disciplanes.get(i).getName(), Database.disciplanes.get(i).getSizeProfessors(),
					Database.disciplanes.get(i).getLengthProfessors(), Database.disciplanes.get(i).getSizeTurmas()
			});
		}
